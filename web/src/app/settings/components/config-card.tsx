...
export function ConfigCard() {
  ...
  const syncImagesToStorage = useSettingsStore((state) => state.syncImagesToStorage);
  ...
  const showLegacyWebDAVFields = imageStorageMode === "webdav" || (imageStorageMode === "both" && !config?.image_storage?.s3_endpoint && Boolean(config?.image_storage?.webdav_url));
  const showS3Fields = imageStorageMode === "s3" || (imageStorageMode === "both" && !showLegacyWebDAVFields);
  const imageStorageModeLabel = !config?.image_storage?.enabled
    ? "仅本机"
    : imageStorageMode === "both"
      ? showLegacyWebDAVFields ? "本机 + WebDAV" : "本机 + S3"
      : imageStorageMode === "s3"
        ? "仅 S3"
        : imageStorageMode === "webdav"
          ? "仅 WebDAV"
          : "仅本机";
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <div className="space-y-4 rounded-xl border border-stone-200 bg-white px-4 py-3 md:col-span-2">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Button
                  ...
                  variant="outline"
                  className="h-9 rounded-xl border-stone-200 bg-white px-4 text-stone-700"
                  onClick={() => void syncImagesToStorage()}
            ...
            <p className="text-xs leading-6 text-stone-500">
              生成时只处理本次新图片；全量同步用于把已有本地图片补传到当前远端存储。
            </p>
            ...
            <div className="grid gap-4 md:grid-cols-3">
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="s3">仅 S3</SelectItem>
                    <SelectItem value="both">本机 + S3</SelectItem>
              ...
              {showS3Fields ? (
                <>
                  <div className="space-y-2 md:col-span-2">
                    <label className="text-sm text-stone-700">S3 Endpoint</label>
                    <Input
                      value={String(config?.image_storage?.s3_endpoint || "")}
                      onChange={(event) => setImageStorageField("s3_endpoint", event.target.value)}
                  ...
                  <div className="space-y-2">
                    ...
                    <Input
                      value={String(config?.image_storage?.s3_region || "auto")}
                  ...
                  <div className="space-y-2 md:col-span-3">
                    <label className="text-sm text-stone-700">S3 对象前缀</label>
                    <Input
